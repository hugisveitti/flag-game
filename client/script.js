...
var africa = [], americas = [], europe = [], asia = [], oceania = [];
var html = [];
var rnd = 0;
var score = 0;
var boolCap = false;
...
var fourCard = 4;
  ...
  html.push(`
    <div class="row">
    <div class="col s6">
      <div id="1" onclick="fourFlags(` + array[0] +`,` + oneCard + `)" class="center-align card-panel blue hoverable">
        <span class="white-text flow-text truncate">` + one +`
        </span>
      </div>
    </div>
  <div class="col s6">
    <div id="2" onclick="fourFlags(`+ array[1] + `,` + twoCard + `)" class="center-align card-panel blue hoverable">
      <span  class="white-text flow-text truncate">` + two +`
      </span>
    </div>
  </div>

  <div class="col s6">
    <div id="3" onclick="fourFlags(` + array[2]  + `,` + threeCard +`)" class="center-align card-panel blue hoverable">
      <span  class="white-text flow-text truncate">` + three +`
      </span>
    </div>
  </div>

  <div class="col s6">
    <div id="4" onclick="fourFlags(` + array[3]  + `,` + fourCard +`)" class="center-align card-panel blue hoverable">
      <span class="white-text flow-text truncate">` + four +`
      </span>
    </div>
  </div>
</div>
...
//this is possible because we shuffle the array in the beginning
  ...
  //if wrong answer then display how many wrong
      ...
      if(gameSettings === "all"){
        html.push(`
          <div class="center-align">
            <a id="wrongButton" class="button btn">Score `+ score +`</a>
            <a id="bonusButton" class="button btn">`+ bonus +`x</a>
          </div>
          <br>
          `);
      }
      //else {
        html.push(`
          <div class="center-align">
              <a id="rowButton" class="button btn">Right in a Row `+ ansRow +`</a>
          </div>
          <br>
          `);
      //}
...
function fourFlags(num, numCard){
  ...
  if(num === rnd){
    ...
    $(idCard).removeClass("center-align card-panel blue hoverable").addClass("center-align card-panel green hoverable");
    ...
    if(gameSettings != "random"){
      if(ansRow % 5 == 0){
        bonus = bonus * 2;
        Materialize.toast(ansRow + " Right Answers in a Row", 1500,'' ,function(){
          Materialize.toast("Right Answer Now Gives " + bonus + " points", 1500);
        });
      }
      rnd++;
      $("#randButt").text(rnd + "/" + countries.length);
    ...
    $(idCard).removeClass("center-align card-panel blue hoverable").addClass("center-align card-panel red hoverable");
    if(bonus != 1){
    ...
    <div class="center-align">
        <div class="col s12 m6">
          <div class="card-image">
            <div class="card-content white-text">
              <img class="z-depth-3" width="200" src="` + countries[rnd].flag + `">
            </div>
...
console.log("rnd is " + rnd);
    ...
    for(var j=0; j<countries[rnd].altSpellings.length; j++){
      ...
      if(name.toUpperCase() === countries[rnd].altSpellings[j].toUpperCase()){
        boolCount = true;
        //check if both capital and country are correct
        checkCorrect();
      }
...
//check if capital is correct
  $("#country_capital").keyup(function(){
...
//add the name of the country
    ...
    if(stilling === "four"){
      ...
      if(gameSettings === "all"){
        html.push(`
          <div class="center-align">
            <a id="wrongButton" class="button btn">Wrong Answers `+ wrongCount +`</a>
          </div>
          `);
      }
      ...
      <div class="col s12 m6">
        <div class="card green z-depth-3">
          <div class="card-content white-text">
            ...
            <p>You finished the game with ` + score + ` points</p>
          </div>
          <div class="card-action">
...
for(var i = 0; i < highscoreArray.length; i++){
  html.push(`
    <tr>
      <td>` + highscoreArray[i][1] + `</td>
      <td>` + highscoreArray[i][0] + `</td>
    </tr>
    `);
}
